    ...
    async execute(rawOldMsg, rawNewMsg) {
        try {
            ...
            try {
                ...
                guildOwner.send({
                    content: `The bot attempted to log a 'messageUpdate' event trigger but the 'logChannelID' setting is not configured. Please use /configupdate in the respective server (Server_ID: ${guild.id}) to fix the issue. Thank you! <3`
                }).catch((error) => {
                    return console.log("Failed to notify the owner. Possibly has DM's disabled. Error Type: " + error.message);
                })
            } 
        } catch(err) {

  ...
  async execute(interaction) {
    try {
      ...
      if (interaction.user.equals(specifiedUser)) {
        // If the specified user = triggering user, end and notify user.
        return interaction.reply({
          content: "You can't timeout yourself!",
          ephemeral: true,
        });
      } else if (client.user.equals(specifiedUser)) {
        // Again with targeting the bot. When will you learn?
        return interaction.reply({
      ...
      await specifiedGuildMember.timeout(duration, [reason]).catch((error) => {
        ...
        if (error.code == 50013) {
          if (!specifiedGuildMember.manageable) {
            ...
            return interaction.reply({
              content: `The specified user (${specifiedUser.tag}) is higher than me in the hierarchy and therefor cannot be timed out by me.`,
              ephemeral: true,
            });
          }
        } else {

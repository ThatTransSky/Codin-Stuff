    ...
    try {
      ...
      if (!interaction.replied) {
        if (specifiedGuildMember.isCommunicationDisabled()) {
          ...
          let timeoutEndsMilliseconds =
            specifiedGuildMember.communicationDisabledUntil - Date.now();
          let timeoutEndsSeconds = Math.trunc(timeoutEndsMilliseconds / 1000);
          ...
          timeoutEndsMilliseconds %= 1000;
          timeoutEndsSeconds %= 60;
          timeoutEndsMinutes %= 60;
          timeoutEndsHours %= 24;
          return interaction.reply({
            content: `${specifiedUser} is timed out for ${timeoutEndsDays} days, ${timeoutEndsHours} hours, ${timeoutEndsMinutes} minutes, ${timeoutEndsSeconds} seconds and ${timeoutEndsMilliseconds} milliseconds.`,
          });
        } else {
          return interaction.reply({
